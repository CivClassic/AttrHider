	...
	private void registerPacketListeners() {
		...
		protocolManager.addPacketListener(new PacketAdapter(this, PacketType.Play.Server.ENTITY_EFFECT) {
			...
			public void onPacketSending(PacketEvent e) {
				try {
					PacketContainer p = e.getPacket();
					if (e.getPlayer().getEntityId() != p.getIntegers().read(0)) { //Don't hide a player's own stuff
						// from them
		...
		ProtocolLibrary.getProtocolManager().addPacketListener(
				new PacketAdapter(this, ListenerPriority.NORMAL, new PacketType[]{PacketType.Play.Server
						.ENTITY_METADATA}) {
					...
					public void onPacketSending(PacketEvent event) {
						try {
							...
							//If the entity is not the observer, and the entity is alive, and the entity is not a
							// dragon or wither,
							//and the entity is not the observer's mount
							if ((entity != null) && (observer != entity) && ((entity instanceof LivingEntity)) &&
									(!(entity instanceof EnderDragon) && !(entity instanceof Wither)) && (entity
									.getPassenger() != observer)) {
